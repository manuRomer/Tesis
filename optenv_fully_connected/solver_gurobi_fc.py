...
def construir_inecuaciones_gurobi(adyacencias, R, pi, num_acciones, gamma, limite_min, limite_max):
    modelo = gp.Model("OptEnv_Gurobi_FC")
    modelo.setParam('OutputFlag', 0)
    V = {}
    M = limite_max - limite_min
    
    for s in adyacencias:
        V[s] = modelo.addVar(vtype=GRB.CONTINUOUS, lb=limite_min, ub=limite_max, name=f"V_{s}")
        
    for s, vecinos in adyacencias.items():
        # Cota Superior
        z_upper = []
        for s_next in vecinos:
            z = modelo.addVar(vtype=GRB.BINARY, name=f"z_up_{s}_{s_next}")
            z_upper.append(z)
        ...
        for a in range(num_acciones):
            y_lower = []
            for s_next in vecinos:
                y = modelo.addVar(vtype=GRB.BINARY, name=f"y_low_{s}_{a}_{s_next}")
                y_lower.append(y)
